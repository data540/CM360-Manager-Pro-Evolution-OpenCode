...
const PlacementGrid: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950/40">
      <div className="relative z-40 p-4 border-b border-slate-800 flex items-center justify-between gap-4 bg-slate-900/50 backdrop-blur-sm">
              ...
              <div className="relative z-[130]">
                ...
                {isBulkActionsOpen && (
                  <div className="absolute right-0 top-6 w-52 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-[200] py-1 pointer-events-auto">
                    <button onClick={() => { setIsBulkNamingOpen(true); setIsBulkActionsOpen(false); }} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-blue-600 hover:text-white flex items-center gap-2">
                      <Zap className="w-3.5 h-3.5" /> Bulk naming
                    </button>
                    <button onClick={() => { handleDuplicateSelected(); }} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-blue-600 hover:text-white flex items-center gap-2">
                      <Copy className="w-3.5 h-3.5" /> Duplicate
                    </button>
                    <button onClick={handleAddIdToNaming} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-blue-600 hover:text-white flex items-center gap-2">
                      <Layers className="w-3.5 h-3.5" /> Add placement ID
                    </button>
                    <div className="px-3 py-2 border-t border-slate-800 mt-1">
                      <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Set status</p>
                      <div className="grid grid-cols-2 gap-1">
                        <button onClick={() => applyBulkStatus('Active')} className="px-2 py-1 text-[10px] rounded bg-slate-800 text-slate-300 hover:bg-emerald-600 hover:text-white">Active</button>
                        <button onClick={() => applyBulkStatus('Paused')} className="px-2 py-1 text-[10px] rounded bg-slate-800 text-slate-300 hover:bg-amber-600 hover:text-white">Paused</button>
                        <button onClick={() => applyBulkStatus('Draft')} className="px-2 py-1 text-[10px] rounded bg-slate-800 text-slate-300 hover:bg-blue-600 hover:text-white">Draft</button>
                        <button onClick={() => applyBulkStatus('Completed')} className="px-2 py-1 text-[10px] rounded bg-slate-800 text-slate-300 hover:bg-indigo-600 hover:text-white">Completed</button>
                      </div>
                    </div>
                    <button 
                      onClick={() => {
                      ...
                      className="w-full text-left px-3 py-2 text-xs text-rose-400 hover:bg-rose-600/20 flex items-center gap-2 border-t border-slate-800 mt-1"
                    >
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          ...
          <thead className="sticky top-0 z-10">
            <tr className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
              ...
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">
              ...
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">Site<div className="absolute right-0 top-0 h-full w-1 cursor-col-resize" onMouseDown={(e) => startResize('site', e.clientX, columnWidths.site)} /></th>
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">Dimensions<div className="absolute right-0 top-0 h-full w-1 cursor-col-resize" onMouseDown={(e) => startResize('dimensions', e.clientX, columnWidths.dimensions)} /></th>
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">Type<div className="absolute right-0 top-0 h-full w-1 cursor-col-resize" onMouseDown={(e) => startResize('type', e.clientX, columnWidths.type)} /></th>
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">Status<div className="absolute right-0 top-0 h-full w-1 cursor-col-resize" onMouseDown={(e) => startResize('status', e.clientX, columnWidths.status)} /></th>
              <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 relative">Flight Dates<div className="absolute right-0 top-0 h-full w-1 cursor-col-resize" onMouseDown={(e) => startResize('dates', e.clientX, columnWidths.dates)} /></th>
