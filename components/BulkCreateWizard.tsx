...
const BulkCreateWizard: React.FC<BulkCreateWizardProps> = ({ onClose }) => {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Name', 'Site ID', 'Size'];
    const example = ['Youtube_Video_Hero', '8959852', 'Video'];
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), example.join(',')].join('\n');
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "cm360_bulk_template.csv");
  ...
  const finalize = () => {
    ...
    const newPlacements: Placement[] = batchItems.map(item => ({
      ...
      size: item.size,
      type: type,
      compatibility: type === 'Video' ? 'IN_STREAM_VIDEO' : 'DISPLAY',
      status: 'Draft',
          ...
          {step === 2 && (
            <div className="space-y-4 animate-in slide-in-from-bottom-4">
              <div className="flex justify-between items-end">
                <div>
                  <h3 className="text-lg font-bold text-slate-200">Import Placement Data</h3>
                  <p className="text-xs text-slate-500">Upload a CSV or paste from Excel (Name | Site | Size)</p>
                </div>
                <div className="flex gap-4">
                  <button 
                    onClick={handleDownloadTemplate}
                    className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase hover:text-white transition-colors"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-4">
                  <div className="p-6 bg-slate-950 border border-dashed border-slate-800 rounded-2xl flex flex-col items-center justify-center text-center group hover:border-blue-500/50 transition-all">
                    <div className="w-12 h-12 bg-blue-600/10 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                      <Upload className="w-6 h-6 text-blue-500" />
                    </div>
                    <p className="text-xs font-bold text-slate-300 mb-1">Upload CSV</p>
                    <p className="text-[10px] text-slate-500 mb-4">Drag & drop or click to browse</p>
                    <input 
                      type="file" 
                      accept=".csv,.txt" 
                    ...
                    <label 
                      htmlFor="csv-upload"
                      className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-[10px] font-bold rounded-lg cursor-pointer transition-all"
                    >
                      Browse Files
                    </label>
                  </div>
                  ...
                  <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Format Guide</span>
                    </div>
                    <ul className="text-[10px] text-slate-600 space-y-1.5 list-disc pl-4">
                      <li>Column 1: Placement Name</li>
                      <li>Column 2: Site ID (Numeric)</li>
                      <li>Column 3: Size (e.g. 300x250)</li>
                    </ul>
                  </div>
                </div>
              </div>
